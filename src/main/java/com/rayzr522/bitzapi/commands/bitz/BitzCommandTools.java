...
public class BitzCommandTools implements BitzCommand {
	...
	public boolean execute(CommandSender sender, String[] args, BitzPlugin plugin) {
		...
		if (args.length < 1) {
			...
			player.getInventory().addItem(BitzTools.createTool(Material.GOLD_NUGGET, ToolType.REGION_TOOL), BitzTools.createTool(Material.STICK, ToolType.LOCATION_TOOL), BitzTools.createTool(Material.BLAZE_ROD, ToolType.LOCATION_LIST_TOOL));
